...
export const DataProvider = ({children}) => {
  ...
  const updatePost = async (updatePost,content, images) => {
    ...
    try {
      const response = await axios.patch(`${BASE_URL}/post/${updatePost}`, {
        content, images
  ...
  const likePost =async(_id, likes) => {
    try {
      const response = await axios.patch(`${BASE_URL}/post/${_id}/like`, {
        likes
      }, {
        headers: {Authorization: userToken},
      });
  ...
  const unLikePost =async(_id, likes) => {
    try {
      const response = await axios.patch(`${BASE_URL}/post/${_id}/unlike`, {
        likes
      }, {
        headers: {Authorization: userToken},
      });
  ...
  const getUserPosts = async () => {
    ...
    try {
      const response = await axios.get(`${BASE_URL}/user_posts/${id}`, {
        headers: {Authorization: userToken},
      });    
